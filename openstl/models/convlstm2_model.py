...
from openstl.utils import reshape_patch_back, reshape_patch



class ConvLSTM2_Model(nn.Module):
    r"""ConvLSTM Model
    ...
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        super(ConvLSTM2_Model, self).__init__()
        ...
        self.position_embedding = AxialPositionalEmbedding(dim = 1, shape = (64, 64))
        self.temporal_agg = nn.Sequential(
            *[
                AxialAttention(dim=1, dim_index=1, heads=64, num_dimensions=2)
                for _ in range(self.num_att_layers)
            ]
        )


    # torch.Size([B, 1, 64, 64])
    def next_deal(self, res):
    ...
    def forward(self, frames_tensor, mask_true, **kwargs):
        # 传进来的 frames_tensor 是分过patch的, 应该是[B, 23, 8, 8, 64]
        ...
        # 输入 reshape_patch_back 的 shape 应该是 [B, T, H, W, C]
        next_frames = reshape_patch_back(next_frames, 8)                # 应该是 [B, 23, 64, 64, 1]
        
        # [B, T, H, W, C] --> [B, T, C, H, W]
        next_frames = next_frames.permute(0, 1, 4, 2, 3).contiguous()   # 应该是 [B, 23, 1, 64, 64]
        ...
        for t in range(seq_len):
            ...
            # print('cur_input_1', cur_input.shape)     # torch.Size([B, 1, 64, 64])
            cur_input = self.next_deal(cur_input)       # torch.Size([B, 1, 64, 64])
        ...
        final_output = final_output.permute(0, 1, 3, 4, 2).contiguous()   # torch.Size([B, 23, 64, 64, 1])
        # 分patch的输入要求：[batch, length, height, width, channel]
        final_output = reshape_patch(final_output, 8)                     # torch.Size([B, 23, 8, 8, 64])
