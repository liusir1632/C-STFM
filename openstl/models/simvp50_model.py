...
from openstl.modules.simvp50_modules import TAUSubBlock

class SimVP50_Model(nn.Module):
    r"""SimVP Model
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        super(SimVP50_Model, self).__init__()
        ...
        if model_type == 'incepu':
            ...
            # self.hid = TCN(input_size=32, output_size=32, num_channels=[64, 64, 64, 64], kernel_size=3, dropout=0.1)
    ...
    def forward(self, x_raw, **kwargs):
        ...
        # # print('z.shape', z.shape)     # torch.Size([64, 12, 32, 32, 32])
        
        # b,t,c,h,w = z.shape
        # z = z.permute(0,3,4,1,2)
        # z = z.reshape(-1,t,c)  
        
        # future_time_steps = 12
        # predictions = []
              
        # # 迭代进行多个时间步的预测
        # for _ in range(future_time_steps):
        #     # 获取当前时间步的预测结果
        #     current_prediction = self.hid(z)
            
        #     # 将当前预测结果加入到列表中
        #     predictions.append(current_prediction.unsqueeze(1))  # 在时间维度上增加一个维度

        #     # 更新输入数据，将当前预测结果作为下一个时间步的输入
        #     z = torch.cat([z[:, 1:, :], current_prediction.unsqueeze(1)], dim=1)

        # # 将所有预测结果连接成一个张量
        # final_predictions = torch.cat(predictions, dim=1)              
              
        # final_predictions = final_predictions.reshape(b,h,w,t,c)
        # hid = final_predictions.permute(0,3,4,1,2)  
        # # print('hid.shape', hid.shape)   # torch.Size([64, 12, 32, 32, 32])
