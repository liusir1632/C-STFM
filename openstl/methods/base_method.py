...
class Base_method(object):
    ...
    def __init__(self, args, device, steps_per_epoch):
        ...
        if 'weather' in self.args.dataname:
            self.metric_list, self.spatial_norm = ['mse', 'rmse', 'mae', 'ssim','acc'], True
        else:
    ...
    def _dist_forward_collect(self, data_loader, length=None, gather_data=False):
        ...
        # if self.args.data_name == "weather_mv6_0_25":
        #     fraction = 0.6
        # else:
            # fraction = 1
        ...
        for idx, (batch_x, batch_y) in enumerate(data_loader):
            ...
            # 新增参数 fraction 用于控制保存率，由于多变量情况数据过大，全部保存会导致内存溢出，所以使用保存率参数
            if idx >= total_batches * fraction:
                break
            ...
            with torch.no_grad():
                ...
                # print("pred_y.shape", pred_y.shape)         # torch.Size([32, 10, 1, 64, 64]) moving-mnist数据集 上面三个输出都一样
