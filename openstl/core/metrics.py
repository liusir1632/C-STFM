...
def ACC(pred, true, spatial_norm=False):
    pred_bias = pred - np.mean(pred, axis=(0, 1))
    true_bias = true - np.mean(true, axis=(0, 1))

    acc1 = np.sum(pred_bias * true_bias)
    acc2 = np.sum(np.power(pred_bias, 2))
    acc3 = np.sum(np.power(true_bias, 2))

    if not spatial_norm:
        result = acc1 / np.sqrt(acc2 * acc3)
    else:
        # norm = pred.shape[-1] * pred.shape[-2] * pred.shape[-3]
        # result = acc1 / (norm * np.sqrt(acc2 * acc3))
        result = acc1 / np.sqrt(acc2 * acc3)
...
def MSE(pred, true, spatial_norm=False):
    # print('pred.shape', pred.shape)     # true.shape (4, 12, 1, 64, 64)
    # print('true.shape', true.shape)     # pred.shape (4, 12, 1, 64, 64)
    if not spatial_norm:
...
def metric(pred, true, mean=None, std=None, metrics=['mae', 'mse'],
    ...
    if 'acc' in metrics:
        if channel_names is None:
            eval_res['acc'] = ACC(pred, true, spatial_norm) 

    if 'mse' in metrics:
        # 检查变量 channel_names 是否为 None。如果是，表示没有指定通道名称，
        # 则直接计算整体预测值 pred 和真实值 true 之间的均方误差，并将结果存储在 eval_res 字典的键 'mse' 下
        if channel_names is None:
            eval_res['mse'] = MSE(pred, true, spatial_norm)
        else:
            # 如果 channel_names 不为 None，表示指定了通道名称，进入 else 分支：
            # a. 初始化变量 mse_sum 为 0，用于累积每个通道的均方误差。
            # b. 使用循环遍历通道名称列表 channel_names 中的每个通道：
            # i. 构造通道相关的预测值和真实值的切片，这是通过对 `pred` 和 `true` 进行切片操作实现的。切片的范围是 `i*c_width` 到 `(i+1)*c_width`。
            # ii. 计算每个通道的均方误差，将结果存储在 `eval_res` 字典的键 `'mse_{str(c_name)}'` 下，其中 `{str(c_name)}` 是通道名称的字符串表示。
            # iii. 将当前通道的均方误差累加到 `mse_sum` 中。
            mse_sum = 0.
            ...
            # c. 计算所有通道的均方误差的平均值，存储在 eval_res 字典的键 'mse' 下，除数是通道数目 c_group。
