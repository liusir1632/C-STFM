...
class SAFM(nn.Module):
    ...
    def forward(self, x):
        ...
        B, T_C, H, W = x.shape
        C = T_C // 12
        x = x.reshape(B,12,C,H,W)

        # 将通道平均分为n_levels份,n_levels是尺度的个数: (B,C,h,w) --chunk--> (B,C/n_levels,h,w)
        xc = list(x.chunk(self.n_levels, dim=1))     # xc[0] 的 shape 为 [B, 3, C, H, W]
        
        for i in range(self.n_levels):
            xc[i] = xc[i].reshape(B, 3*C, H, W)
        
        xc = tuple(xc)

        out = []
        
        # y0, skip0 = self.mfr3(xc[0])
        # out.append(y0)
        
        # y1, skip1 = self.mfr3(xc[1] + skip0)
        # out.append(y1)
        
        # y2, skip2 = self.mfr3(xc[2] + skip1)
        # out.append(y2)
        
        # y3, _ = self.mfr3(xc[3] + skip2)
        # out.append(y3)
        ...
        # y1, _ = self.mfr3(xc[1] + y0)
        # out.append(y1)
        
        # y2, _ = self.mfr3(xc[2] + y1)
        # out.append(y2)
        
        # y3, _ = self.mfr3(xc[3] + y2)
        # out.append(y3)  
        
        _, skip0 = self.mfr3(xc[0])
        out.append(skip0)
        
        _, skip1 = self.mfr3(xc[1])
        out.append(skip1)
        
        _, skip2 = self.mfr3(xc[2])
        out.append(skip2)
        
        _, skip3 = self.mfr3(xc[3])
        out.append(skip3)              
