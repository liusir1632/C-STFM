...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        x_w = self.pool_w(group_x).permute(0, 1, 3, 2) # 使用全局平均池化压缩 时序维度
...
class SAFM(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, self.n_levels):
            # 计算当前层级的skip
            skip = self.mfr3(xc[i] + skip)
            # 将结果添加到out列表中
            out.append(skip)

        # out = []
        
        # skip0 = self.mfr3(xc[0])
        # out.append(skip0)
        
        # skip1 = self.mfr3(xc[1] + skip0)
        # out.append(skip1)
        
        # skip2 = self.mfr3(xc[2] + skip1)
        # out.append(skip2)
        
        # skip3 = self.mfr3(xc[3] + skip2)
        # out.append(skip3)              
...
class GASubBlock(nn.Module):
    ...
    def __init__(self, dim, kernel_size=21, mlp_ratio=4.,
        ...
        # moving-mnist
        self.safm_drop = DropPath(0.4)
        self.ema_drop  = DropPath(0.4)        
        self.mlp_drop = DropPath(drop_path1) if drop_path1 > 0. else nn.Identity()        
    ...
    def forward(self, x):
        x = x + self.ema_drop(
            self.layer_scale_3.unsqueeze(-1).unsqueeze(-1) * self.EMA(self.norm3(x)))
                
        x = x + self.safm_drop(
            self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.safm(self.norm1(x)))
