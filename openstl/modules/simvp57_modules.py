...
class BasicConv2d(nn.Module):
    ...
    def forward(self, x):
        # print('in_channels, out_channels*4', self.in_channels, self.out_channels*4)
        # print("x.shape", x.shape)       # torch.Size([12, 32, 32, 32])
        y = self.conv(x)
        # print("y.shape", y.shape)       # torch.Size([12, 32, 64, 64])
        if self.act_norm:
...
class GASubBlock(nn.Module):
    ...
    def __init__(self, dim, kernel_size=21, mlp_ratio=4.,
        ...
        self.drop_path_tau = DropPath(0.05) if drop_path > 0. else nn.Identity()
        self.drop_path_sau = DropPath(0.05) if drop_path > 0. else nn.Identity()
        self.drop_path_tcn = DropPath(0.05) if drop_path > 0. else nn.Identity()
        ...
        self.spatial = SpatialAttention_CBAM()
        self.reduction = max(dim // 16, 4)
        ...
        self.hid = TCN(input_size=int(dim/self.T), output_size=int(dim/self.T), num_channels=[int(dim/self.T), int(dim/self.T)], kernel_size=3, dropout=drop)
    ...
    def spat_atten(self,x):
        # u = x.clone()
        b, c, _, _ = x.size()
        
        spa_atten = self.avg_pool(x).view(b, c)
        spa_atten = self.fc(spa_atten).view(b, c, 1, 1)
        sigmoid_spa_atten = self.sigmoid(spa_atten)        
      
        return (self.spatial(x) * sigmoid_spa_atten)
    

    # x.shape [B, T*C, H, W]   经过这个输出要shape保持不变
    def tem_conv(self, x):
        
        # print("x.shape", x.shape)       # torch.Size([1, 120, 16, 16])
        ...
        x = x.permute(0,3,4,1,2).reshape(-1, T, C)   
        
        x = self.hid(x)
        x = x.reshape(B, H, W, T, C)
        x = x.permute(0,3,4,1,2).reshape(B, T*C, H, W)
        
        x = x1 + self.drop_path_tcn(
            self.layer_scale_3.unsqueeze(-1).unsqueeze(-1) * x)         
        return x
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path_tau(
            self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.attn(self.norm1(x)))
        
        x = x + self.drop_path_sau(self.layer_scale_3.unsqueeze(-1).unsqueeze(-1) * self.spat_atten(self.norm3(x)))
...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, _, _ = x.size()
        avg_atten = self.avg_pool(x).view(b, c)
        avg_atten = self.fc(avg_atten).view(b, c, 1, 1)
        max_atten = self.max_pool(x).view(b, c)
        max_atten = self.fc(max_atten).view(b, c, 1, 1)
        se_atten= self.sigmoid(max_atten + avg_atten)     
        sig_f_x = self.sigmoid(f_x)   
        Y = se_atten * f_x * u + (max_atten + avg_atten) * sig_f_x * u
        return Y
