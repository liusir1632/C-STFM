...
class EMA(nn.Module):
    def __init__(self, channels, c2=None, factor=2):
        ...
        self.conv1x1 = nn.Conv2d(1024, 1024, kernel_size=1, stride=1, padding=0)
        # self.conv3x3 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=3, stride=1, padding=1)
        self.conv3x3 = nn.Conv2d(channels // 3 // 6, channels // 3 // 6, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        #(B, T*C, H, W)
        b, c, h, w = x.size()
        # print("x1.shape", x.shape)          # torch.Size([10, 288, 32, 32])
        ...
        # print("x2.shape", x.shape)          # torch.Size([10, 12, 24, 1024])       
        
        temp = x.reshape(b * 3 * 6, new_t, new_c, h, w).reshape(b * 3 * 6, new_t*new_c, h, w)
        # print("temp.shape", temp.shape)     # torch.Size([180, 16, 32, 32])
        
        x = x.permute(0,3,1,2)
        # print("x3.shape", x.shape)          # torch.Size([10, 1024, 12, 24])  [B, H*W, T, C]
        
        ### 通道空间注意力模块  ###
        
        group_x = x.reshape(b * 3 * 6, h*w, new_t, new_c)  # 在通道方向上将输入分为 2 组
        
        
        # print("group_x.shape", group_x.shape)       # torch.Size([180, 1024, 4, 4])
       
        x_h = self.pool_h(group_x) # 使用全局平均池化压缩 通道 维度
        # print("x_h.shape", x_h.shape)               # torch.Size([180, 1024, 4, 1])
        
        x_w = self.pool_w(group_x).permute(0, 1, 3, 2) # 使用全局平均池化压缩 时序 维度
        # print("self.pool_w(group_x).shape", self.pool_w(group_x).shape)  # torch.Size([180, 1024, 1, 4])
        # print("x_w.shape", x_w.shape)               # torch.Size([180, 1024, 4, 1])
        
        hw = self.conv1x1(torch.cat([x_h, x_w], dim=2))# 将 时序 方向和 通道 方向的全局特征进行拼接， 然后通过1×1Conv进行变换,来编码 时序 和空间 维度的特征
        # print("hw.shape", hw.shape)                 # torch.Size([180, 1024, 8, 1])
        
        x_h, x_w = torch.split(hw, [new_t, new_c], dim=2) # 沿着 时序和通道 方向将其分割为两个矩阵表示
        # print("x_h.shape", x_h.shape)               # torch.Size([180, 1024, 4, 1])
        # print("x_w.shape", x_w.shape)               # torch.Size([180, 1024, 4, 1])       
        
        x1 = self.gn(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid()) 
        # print("x1.shape", x1.shape)                 # torch.Size([180, 1024, 4, 4])
        x1 = x1.reshape(b*3*6, h, w, new_t*new_c).permute(0,3,1,2)
        # print("x1.shape", x1.shape)                 # torch.Size([180, 16, 32, 32])
        
        x2 = self.conv3x3(temp) # 通过3×3卷积提取局部上下文信息: (B*G,C/G,H,W)-->(B*G,C/G,H,W)
        # print("x2.shape", x2.shape)                 # torch.Size([180, 16, 32, 32])
        ...
        x11 = self.softmax(self.agp(x1).reshape(b*3*6, -1, 1).permute(0, 2, 1)) 
        # x11 = self.softmax(self.agp(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1)) 
        # print("x11.shape", x11.shape)               # torch.Size([180, 1, 16])
        
        # 将3×3分支的输出进行变换,以便与1×1分支生成的通道描述符进行相乘: (B*G,C/G,H,W)-->reshape-->(B*G,C/G,H*W)
        x12 = x2.reshape(b*3*6, new_t*new_c, -1) 
        # x12 = x2.reshape(b * self.groups, c // self.groups, -1)  
        # print("x12.shape", x12.shape)               # torch.Size([180, 16, 1024])
        
        y1 = torch.matmul(x11, x12) # (B*G,1,C/G) @ (B*G,C/G,H*W) = (B*G,1,H*W)
        # print("y1.shape", y1.shape)                 # torch.Size([180, 1, 1024])
        ...
        x21 = self.softmax(self.agp(x2).reshape(b * 3 * 6, -1, 1).permute(0, 2, 1)) 
        # print("x21.shape", x21.shape)               # torch.Size([180, 1, 16])
        
        # b*g, c//g, hw  # 将1×1分支的输出进行变换,以便与3×3分支生成的通道描述符进行相乘: (B*G,C/G,H,W)-->reshape-->(B*G,C/G,H*W)
        x22 = x1.reshape(b * 3 * 6, new_t * new_c, -1)  
        # print("x22.shape", x22.shape)               # torch.Size([180, 16, 1024])
        
        y2 = torch.matmul(x21, x22)  # (B*G,1,C/G) @ (B*G,C/G,H*W) = (B*G,1,H*W)
        # print("y2.shape", y2.shape)                 # torch.Size([180, 1, 1024])
        
        # 聚合两种尺度的空间位置信息, 通过sigmoid生成空间权重, 从而再次调整输入表示
        weights = (y1+y2).reshape(b * 3 * 6, 1, h, w)  # 将两种尺度下的空间位置信息进行聚合: (B*G,1,H*W)-->reshape-->(B*G,1,H,W)
        weights_ =  weights.sigmoid() # 通过sigmoid生成权重表示: (B*G,1,H,W)
        
        # print("temp.shape", temp.shape)             # torch.Size([180, 16, 32, 32])
        # print("weights_.shape", weights_.shape)     # torch.Size([180, 1, 32, 32])
        out = (temp * weights_).reshape(b, c, h, w) # 通过空间权重再次校准输入: (B*G,C/G,H,W)*(B*G,1,H,W)==(B*G,C/G,H,W)-->reshape(B,C,H,W)
        # print("(group_x * weights_).shape", (temp * weights_).shape)     # torch.Size([180, 16, 32, 32])
        # print("out.shape", out.shape)               # torch.Size([10, 288, 32, 32])
        return out
