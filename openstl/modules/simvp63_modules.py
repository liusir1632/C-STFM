...
class GASubBlock(nn.Module):
    ...
    def spat_atten(self,x):
        ...
        # return (self.spatial(x) * sigmoid_spa_atten * u)
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path2(self.layer_scale_3.unsqueeze(-1).unsqueeze(-1) * self.spat_atten(self.norm3(x)))
...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        Y = se_atten * f_x * u + (max_atten + avg_atten) * u * sig_f_x              # TAU新融合方式： X*X静*Sigmoid(通) + X*X通*Sigmoid(静) 
        return Y
