...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("temp.shape", x.shape)          # torch.Size([10, 264, 32, 32])
        
        group_x2 = x.clone()
        x_h = self.pool_h(group_x2) # 使用全局平均池化压缩 水平 方向
        # print("x_h.shape", x_h.shape)           # torch.Size([10, 264, 32, 1])
        
        x_w = self.pool_w(group_x2) # 使用全局平均池化压缩 垂直 空间方向
        # print("self.pool_w(group_x).shape", self.pool_w(group_x2).shape)  # torch.Size([32, 16, 1, 7])
        # print("x_w.shape", x_w.shape)           # torch.Size([10, 264, 32, 1])
        ...
        # print("x2.shape", x.shape)          # torch.Size([10, 12, 22, 1024])       
        ...
        x_c = self.pool_h(group_x) # 使用全局平均池化压缩 通道 方向
        # print("x_c.shape", x_c.shape)               # torch.Size([10, 1024, 12, 1])
        
        x_t = self.pool_w(group_x).permute(0, 1, 3, 2) # 使用全局平均池化压缩 时序方向
        # print("self.pool_w(group_x).shape", self.pool_w(group_x).shape)  # torch.Size([10, 1024, 1, 22])
        # print("x_t.shape", x_t.shape)               # torch.Size([10, 1024, 22, 1])
        
        # print("temp.shape", temp.shape)             # torch.Size([10, 264, 32, 32])
        # print("x_h.shape", x_h.shape)               # torch.Size([10, 264, 32, 1])
        # print("x_w.shape", x_w.shape)               # torch.Size([10, 264, 1, 32])
        
        # 这里得到的 x2 就是使用 坐标注意 修饰过的
        # x3 = self.gn2(temp * x_h.sigmoid() * x_w.sigmoid()) 
        # x3 = temp * x_h.sigmoid() * x_w.sigmoid()
        x3 = x_h.sigmoid() * x_w.sigmoid()
        # print("x3.shape", x3.shape)
        
        # 这里得到的 x1 就是使用 时间注意 和 通道注意 修饰过的
        # x1 = self.gn(group_x * x_c.sigmoid() * x_t.permute(0, 1, 3, 2).sigmoid()) 
        x1 = self.gn(x_c.sigmoid() * x_t.permute(0, 1, 3, 2).sigmoid()) 
        # print("x1.shape", x1.shape)                 # torch.Size([10, 1024, 12, 22])
        
        # 这里得到的 x1 就是使用 时间注意 和 通道注意 修饰过的，再变回(B, T*C, H, W)的shape
        x1_2 = x1.reshape(b, h, w, T*C).permute(0,3,1,2)
        # print("x1.shape", x1_2.shape)                 # torch.Size([10, 264, 32, 32])
        
        # 这里的 x2 就是使用 DW-Conv 卷积过的结果
        x2 = self.conv3x3(temp) # 通过3×3卷积提取局部上下文信息: (B*G,C/G,H,W)-->(B*G,C/G,H,W)
        # print("x2.shape", x2.shape)                 # torch.Size([10, 264, 32, 32])
        
        x2_2 = x2.reshape(b, h, w, T*C).reshape(b, h*w, T, C)
        
        # 使用 时序-通道 修饰 conv
        y1 = x1_2 * x2
        # print("y1.shape", y1.shape)                 # torch.Size([10, 264, 32, 32])
        
        # print("group_x.shape", group_x.shape)       # torch.Size([10, 1024, 12, 22])
        
        # print("x_h.shape", x_c.shape)               # torch.Size([10, 1024, 12, 1])
        # print("x_w.shape", x_t.shape)               # torch.Size([10, 1024, 22, 1])
        # 使用 conv 修饰 时序通道
        # y2 = group_x * x2_2.sigmoid() * x_c * x_t.permute(0, 1, 3, 2)
        y2 = x2_2.sigmoid() * x_c * x_t.permute(0, 1, 3, 2)
        # print("y2.shape", y2.shape)                 # torch.Size([10, 1024, 12, 22])
        
        y2 = y2.reshape(b, h, w, T, C).reshape(b, h, w, T*C).permute(0,3,1,2)
        # print("y2.shape", y2.shape)                 # torch.Size([10, 264, 32, 32])
        
        t_c_out = y1 + y2
        
        # h_w_out = x3 * x2 + temp * x_h * x_w * x2.sigmoid()
        h_w_out = x3 * x2 + x_h * x_w * x2.sigmoid()
