...
class SAFM(nn.Module):
    def __init__(self, dim, n_levels=2):
        ...
        # self.mfr0 = AttentionModule(chunk_dim, kernel_size=21, dilation=3)
        # self.mfr1 = AttentionModule(chunk_dim, kernel_size=21, dilation=3)
        # self.mfr2 = AttentionModule(chunk_dim, kernel_size=21, dilation=3)
        self.mfr1 = AttentionModule(chunk_dim, kernel_size=21, dilation=3)
        self.mfr2 = AttentionModule(chunk_dim, kernel_size=21, dilation=3)
    ...
    def forward(self, x):
        ...
        # 初始化skip变量
        _, skip = self.mfr1(xc[0])
        out.append(skip)
        
        _, skip = self.mfr2(xc[1] + skip)
        out.append(skip)
