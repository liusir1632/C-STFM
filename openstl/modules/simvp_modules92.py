...
class SAFM(nn.Module):
    ...
    def forward(self, x):
        ...
        y1 = self.mfr3(xc[1] + y0)
        out.append(y1)
        
        y2 = self.mfr3(xc[2] + y1)
        out.append(y2)
        
        y3 = self.mfr3(xc[3] + y2)
        out.append(y3)
...
class EMA(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        ...
        self.conv_C = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
        self.conv_T = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
        self.conv_H = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
        self.conv_W = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # print("temp.shape", temp.shape)       # torch.Size([10, 264, 32, 32])
        x_H = self.pool_h(temp)         # 使用全局平均池化压缩 w 维度
        # print("x_H1.shape", x_H.shape)       # torch.Size([10, 264, 32, 1])
        
        # x_H_min, _ = torch.min(temp, dim=3, keepdim=True)
        x_H_max, _ = torch.max(temp, dim=3, keepdim=True)
        
        # print("x_H_min.shape", x_H_min.shape)   # torch.Size([10, 264, 32, 1])
        x_H = torch.cat([x_H, x_H_max], dim=3)
        # print("x_H2.shape", x_H.shape)          # torch.Size([10, 264, 32, 2])
        x_H = x_H.permute(0, 3, 2, 1)
        # print("x_H3.shape", x_H.shape)          # torch.Size([10, 2, 32, 264])
        x_H = self.conv_H(x_H)
        # print("x_H4.shape", x_H.shape)          # torch.Size([10, 1, 32, 264])
        x_H = x_H.permute(0, 3, 2, 1)
        # print("x_H5.shape", x_H.shape)          # torch.Size([10, 264, 32, 1])
        
        x_W = self.pool_w(temp)         # 使用全局平均池化压缩 h 维度
        # print("x_W1.shape", x_W.shape)          # torch.Size([10, 264, 1, 32])
        
        # x_W_min, _ = torch.min(temp, dim=2, keepdim=True)
        x_W_max, _ = torch.max(temp, dim=2, keepdim=True)
        
        # print("x_W_min.shape", x_W_min.shape)   # torch.Size([10, 264, 1, 32])
        x_W = torch.cat([x_W, x_W_max], dim=2)
        # print("x_W2.shape", x_W.shape)          # torch.Size([10, 264, 2, 32])
        x_W = x_W.permute(0, 2, 3, 1)
        # print("x_W3.shape", x_W.shape)          # torch.Size([10, 2, 32, 264])
        x_W = self.conv_W(x_W)
        # print("x_W4.shape", x_W.shape)          # torch.Size([10, 1, 32, 264])
        x_W = x_W.permute(0, 3, 2, 1) 
        # print("x_W5.shape", x_W.shape)          # torch.Size([10, 264, 32, 1])
        
        # 使用 h和w 修饰输入
        x3 = self.gn2(temp * x_H.sigmoid() * x_W.permute(0, 1, 3, 2).sigmoid()) 
        # print("x3.shape", x3.shape)             # torch.Size([10, 264, 32, 32])
        ...
        # x_h_min, _ = torch.min(group_x, dim=3, keepdim=True)
        x_h_max, _ = torch.max(group_x, dim=3, keepdim=True)
        
        # print("x_h_min.shape", x_h_min.shape)       # torch.Size([10, 1024, 12, 1])
        x_h = torch.cat([x_h, x_h_max], dim=3)      
        # print("x_h1.shape", x_h.shape)               # torch.Size([10, 1024, 12, 2])
        x_h = x_h.permute(0, 3, 2, 1)
        # print("x_h2.shape", x_h.shape)               # torch.Size([10, 2, 12, 1024])
        x_h = self.conv_C(x_h)
        # print("x_h3.shape", x_h.shape)               # torch.Size([10, 1, 12, 1024])
        x_h = x_h.permute(0, 3, 2, 1)               
        # print("x_h4.shape", x_h.shape)               # torch.Size([10, 1024, 12, 1])
        
        
        # print("group_x.shape", group_x.shape)       # torch.Size([10, 1024, 12, 22])
        x_w = self.pool_w(group_x) # 使用全局平均池化压缩 时序 维度
        # print("x_w.shape", x_w.shape)               # torch.Size([10, 1024, 1, 22])       
        
        # x_w_min, _ = torch.min(group_x, dim=2, keepdim=True)
        x_w_max, _ = torch.max(group_x, dim=2, keepdim=True)
        
        # print("x_w_min.shape", x_w_min.shape)       # torch.Size([10, 1024, 1, 22])
        x_w = torch.cat([x_w, x_w_max], dim=2)      
        # print("x_w1.shape", x_w.shape)               # torch.Size([10, 1024, 2, 22])
        x_w = x_w.permute(0, 2, 3, 1)
        # print("x_w2.shape", x_w.shape)               # torch.Size([10, 2, 22, 1024])
        x_w = self.conv_T(x_w)
        # print("x_w3.shape", x_w.shape)               # torch.Size([10, 1, 22, 1024])
        x_w = x_w.permute(0, 3, 2, 1)               
        # print("x_w4.shape", x_w.shape)               # torch.Size([10, 1024, 22, 1])    
        ...
        # print("self.agp(x1).shape", self.agp(x1).shape)     # torch.Size([10, 264, 1, 1])
        ...
        # 大核卷积 和 时空注意 的结果（ 时空注意 修饰 大核卷积 ）
        # torch.matmul()标准的矩阵乘法
        y1 = torch.matmul(x11, x12) # (B*G,1,C/G) @ (B*G,C/G,H*W) = (B*G,1,H*W)
        # print("y1.shape", y1.shape)                 # torch.Size([10, 1, 1024])
        
        # print("x3.shape", x3.shape)             # torch.Size([10, 264, 32, 32])
        x31 = self.softmax(self.agp(x3).reshape(b, -1, 1).permute(0, 2, 1)) 
        # print("x31.shpae", x31.shape)       # torch.Size([10, 1, 264])
        
        # 大核卷积 和 h和w修饰 乘积的结果（ h和w 修饰 大核卷积）
        y3 = torch.matmul(x31, x12)                
        # print("y3.shape", y3.shape)         # torch.Size([10, 1, 1024])
        
        # print("self.agp(x2).shape", self.agp(x2).shape)         # torch.Size([10, 264, 1, 1])
        
        # 假设x1是一个4维张量
        # min_out1, _ = torch.min(x2, dim=2, keepdim=True)  # 在第3维上找到最小值，保持维度
        # print("min_out1.shape", min_out1.shape)         # torch.Size([10, 264, 1, 32])
        # min_out, _ = torch.min(min_out1, dim=3, keepdim=True)  # 在第4维上找到最小值，保持维度
        # print("min_out.shape", min_out.shape)           # torch.Size([10, 264, 1, 1])
        ...
        weights = (y1 + y2 + y3).reshape(b, 1, h, w)  # 将两种尺度下的空间位置信息进行聚合: (B*G,1,H*W)-->reshape-->(B*G,1,H,W)
