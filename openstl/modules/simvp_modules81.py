...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        x33 = x3.reshape(b, c, -1)
        # print("x33.shape", x33.shape)               # torch.Size([10, 264, 1024])
        ...
        y2 = torch.matmul(x21, x33)  # (B*G,1,C/G) @ (B*G,C/G,H*W) = (B*G,1,H*W)
        # print("y2.shape", y2.shape)                 # torch.Size([10, 1, 1024])
        
        # 聚合两种尺度的空间位置信息, 通过sigmoid生成空间权重, 从而再次调整输入表示
        weights = (y2 + y3).reshape(b, 1, h, w)  # 将两种尺度下的空间位置信息进行聚合: (B*G,1,H*W)-->reshape-->(B*G,1,H,W)
