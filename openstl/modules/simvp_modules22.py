...
class SAFM(nn.Module):
    def __init__(self, dim):
        ...
        self.mfr1 = AttentionModule1(dim, kernel_size=5, dilation=1)
        self.mfr2 = AttentionModule(dim, kernel_size=11, dilation=2)
        self.mfr3 = AttentionModule(dim, kernel_size=21, dilation=3)
        self.mfr4 = AttentionModule(dim, kernel_size=37, dilation=4)      
    ...
    def forward(self, x):
        ...
        y0, skip0 = self.mfr1(x)
        # out.append(y0)
        
        y1, skip1 = self.mfr2(x + skip0)
        # out.append(y1)
        
        y2, skip2 = self.mfr3(x + skip1)
        # out.append(y2)
        
        y3, _ = self.mfr4(x + skip2)
        # out.append(y3)
        
        # 将四个尺度的输出在通道上拼接,恢复原shape: (B,C,h,w), 然后通过1×1Conv来聚合多个子空间的不同尺度的通道特征:
        
        out = self.aggr(y0 + y1 + y2 + y3 + x)
