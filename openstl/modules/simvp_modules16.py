...
class SAFM(nn.Module):
    def __init__(self, dim, n_levels=4):
        ...
        self.project = nn.Sequential(
            nn.Conv2d(chunk_dim, chunk_dim, 1, bias=False),
        ...
        self.Sigmoid = nn.Sigmoid()
        self.SE1 = oneConv(chunk_dim,chunk_dim,1,0,1)
        self.SE2 = oneConv(chunk_dim,chunk_dim,1,0,1)
        self.SE3 = oneConv(chunk_dim,chunk_dim,1,0,1)
        self.SE4 = oneConv(chunk_dim,chunk_dim,1,0,1)        
    ...
    def forward(self, x):
        ...
        y0 = self.mfr1(xc[0])
        # out.append(y0)
        y1 = self.mfr1(xc[1] + y0)
        # out.append(y1)
        y2 = self.mfr1(xc[2] + y1)
        # out.append(y2)
        y3 = self.mfr1(xc[3] + y2)
        # out.append(y3)
        ...
        out = []
        y0 = self.project(y0 * y0_weight + xc[0])
        y1 = self.project(y1 * y1_weight + xc[1])
        y2 = self.project(y2 * y2_weight + xc[2])
        y3 = self.project(y3 * y3_weight + xc[3])
        out.append(y0)
        out.append(y1)
        out.append(y2)
        out.append(y3)
